...
class AiController extends Controller
{
    ...
    public function customerAssistant(Request $request)
    {
        try {
            ...
            if (!$apiKey) {
                return response()->json(['answer' => 'AI Service Key missing on server.'], 500);
            }
            ...
            // Attempt call to Google using v1 endpoint and fixed model name
            ...
            // Fallback for model name variations
            $error = $response->json('error.message') ?? 'Connection failed';
            Log::error("Gemini v1 Error: " . $error);
