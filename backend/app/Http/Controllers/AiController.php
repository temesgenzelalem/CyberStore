...
class AiController extends Controller
{
    ...
    public function customerAssistant(Request $request)
    {
        try {
            ...
            // Attempting with gemini-1.5-flash (the current standard)
            ...
            // Diagnostic: Try alternate model name
            ...
            if ($responseAlt->successful()) {
                return response()->json(['answer' => $responseAlt->json('candidates.0.content.parts.0.text')]);
            }

            return response()->json(['answer' => "AI Status: " . ($response->json('error.message') ?? 'Unknown Error')], 500);

        } catch (\Exception $e) {
            return response()->json(['answer' => "Server error in AI core: " . $e->getMessage()], 500);
        }
    }
